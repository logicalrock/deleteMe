...
def broker_editor():
    ...
    while True:
        ...
        print("2. Add a new broker")
        print("3. Update a broker's status")
        print("4. Return to main menu")
        choice = input("Select> ").strip()

        if choice == '1':
            for i, b in enumerate(brokers, 1):
                print(f"{i}. {b['name']} | Status: {b.get('status', 'N/A')}")
        elif choice == '2':
            name = input("Broker name: ").strip()
            url = input("Opt-out URL (optional): ").strip()
            email = input("Privacy email (optional): ").strip()
            notes = input("Notes (optional): ").strip()
            covered_by = input("Covered by services (semicolon-separated): ").strip()
            status = input("Status (Pending, Submitted, Completed): ").strip()
            brokers.append({
                'name': name, 'url': url, 'email': email,
                'notes': notes, 'covered_by': covered_by, 'status': status
            })
        elif choice == '3':
            for i, b in enumerate(brokers, 1):
                print(f"{i}. {b['name']} | Status: {b.get('status', 'N/A')}")
            idx = int(input("Select broker number to update: ").strip()) - 1
            if 0 <= idx < len(brokers):
                brokers[idx]['status'] = input(f"New status for {brokers[idx]['name']}: ").strip()
        elif choice == '4':
            break
        else:
            print("Invalid selection.")

    # Save brokers back to CSV
    with BROKERS_FILE.open('w', newline='', encoding='utf-8') as f:
        fieldnames = ['name', 'url', 'email', 'notes', 'covered_by', 'status']
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(brokers)


def auto_mark_coverage(service):
    brokers = load_brokers()
    updated = False
    for b in brokers:
        if service.lower() in b.get('covered_by', '').lower():
            continue
        existing = b.get('covered_by', '')
        if existing:
            b['covered_by'] = existing + ';' + service
        else:
            b['covered_by'] = service
        updated = True
    ...
    if updated:
        with BROKERS_FILE.open('w', newline='', encoding='utf-8') as f:
            ...
            writer.writeheader()
            writer.writerows(brokers)
        print(f"Brokers updated to reflect coverage by: {service}")
